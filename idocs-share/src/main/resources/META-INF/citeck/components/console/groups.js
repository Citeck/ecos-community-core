  ...
  const ecosAddOrRemove = (parent, child, add, successMessage, failureMessage) => {
   ...
   Alfresco.ConsoleGroups = function(htmlId)
   {
      ...
      YAHOO.extend(SearchPanelHandler, Alfresco.ConsolePanelHandler,
      {
         ...
         {
             ecosAddOrRemove(
                 "GROUP_" + parentShortName,
                 fullName,
                 false,
                 parent._msg("message.delete-success", displayName),
                 parent._msg("message.delete-failure", displayName)
             ).then(() => {
         ...
         _removeUser: function ConsoleGroups_SearchPanelHandler__removeUser(groupId, userId, userDisplayName)
         {
             ...
             ecosAddOrRemove(
                 ...
                 userId,
                 false,
                 parent._msg("message.removeuser-success", name),
                 parent._msg("message.removeuser-failure", name)
             ).then(() => {
               // Refresh column browser
               var paths = this.widgets.columnbrowser.get("urlPath");
               this.widgets.columnbrowser.load(paths, true);
      ...
      YAHOO.extend(CreatePanelHandler, Alfresco.ConsolePanelHandler,
      {
            ...
            parent.getParentGroups(shortName,
            {
               fn: function(groups)
               {
                        ...
                        buttons: [
                           {
                              ...
                              handler: function ConsoleGroups__createGroup_confirmOk()
                              {
                                 ...
                                 if (parent.group)
                                 {
                                    me._createGroupAfterExistCheck.call(me, successHandler, true);
                                 }
                                 else
                                 {
                  ...
                  else
                  {
                     ...
                     me._createGroupAfterExistCheck.call(me, successHandler, false);
                  }
               },
               scope: this
          ...
          * @private
          */
         _createGroupAfterExistCheck: function ConsoleGroups_CreatePanelHandler__createGroupAfterExistCheck(successHandler, groupExists)
         {
            ...
            if (groupExists) {
              ...
              if (!parent.group) {
                  Alfresco.util.PopupManager.displayPrompt({
                      title: parent._msg("message.failure"),
                      text: "Parent is not defined"
                  });
                  return;
              }
              return ecosAddOrRemove(
                "GROUP_" + parent.group,
                "GROUP_" + shortName,
                true,
                undefined,
                parent._msg("message.addgroup-failure", shortName)
              ).then(() => {
                successHandler()
              }).catch(e => {
                console.error(e);
            ...
            var actionRecord = Citeck.Records.getRecordToEdit('alfresco/authority-group-manager-action@');
            actionRecord.att('type', "create");
            if (parent.group) {
              actionRecord.att('parent', 'GROUP_' + parent.group);
            }
            actionRecord.att('shortName', shortName);
