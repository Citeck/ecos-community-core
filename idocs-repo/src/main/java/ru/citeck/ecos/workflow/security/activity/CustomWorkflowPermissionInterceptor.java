package ru.citeck.ecos.workflow.security.activity;
...
/**
 * Override {@link org.alfresco.service.cmr.workflow.WorkflowPermissionInterceptor}
 */
public class CustomWorkflowPermissionInterceptor implements MethodInterceptor {
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        if (methodName.equals("getTaskById")) {
            ...
            if (isInitiatorOrAssignee(wt, currentUser) || fromSameParallelReviewWorkflow(wt, currentUser) ||
                isStartTaskOfProcessInvolvedIn(wt, currentUser) || isUserPartOfSpecialGroup(wt, currentUser)) {
                return result;
            } else {
        ...
        if (methodName.equals("updateTask") || methodName.equals("endTask")) {
            ...
            if (isInitiatorOrAssignee(taskToUpdate, currentUser) || isUserPartOfSpecialGroup(taskToUpdate, currentUser)) {
                return invocation.proceed();
            } else {
                throw new AccessDeniedException("Accessing task with id='" + taskId + "' is not allowed for user '" + currentUser + "'");
            }
    ...
    private boolean isUserPartOfSpecialGroup(WorkflowTask wt, String userName) {
        if (wt == null) {
            return true;
        }
        ...
        if (authorityService.getAuthoritiesForUser(userName).contains(GROUP_WORKFLOW_TASK_ADMIN)) {
            return true;
        }
